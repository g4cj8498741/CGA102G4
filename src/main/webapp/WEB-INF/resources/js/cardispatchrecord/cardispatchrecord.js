...
function init() {
			...
			if (id === 'applyrecord') {
				$('#applyrecord tbody').append(`
                    <tr id="${dispatch.dr_no}">
                        ...
                        <td><label>${dispatch.miles_after}</label></td>
                        <td><label class="status">${status[dispatch.dr_approve]}</label></td>
                        <td class="recallBut">${recallBut}</td>
                        <td class="returncar">${returncar}</td>
                    </tr>       
                `)
				...
				if ($('#applyrecordTab_wrapper').length != 0) {
					...
					t.row.add([
						...
						`<label>${dispatch.miles_after}</label`,
						`<label class="status">${status[dispatch.dr_approve]}</label>`,
						`${recallBut}`,
						`${returncar}`
					]).node().id = dispatch.dr_no;
					t.draw(false);
	...
	$('#drStartTime').blur(function() { //驗證日期大於目前時間
