$(document).ready(function () {
    ...
    storexhr.open("get", path+"/storeshow", true);
    storexhr.send(null);
    storexhr.onload = function () {
        if (storexhr.status == 200) {
            ...
            xhr.open('get', `${path}/storeshow?month=${new Date().toISOString().split('T')[0]}&&store=${city}`, true);
            xhr.send(null);
            xhr.onload = function () {
                if (xhr.status == 200) {
    ...
    $("#show").on('change', '#showcity', function (e) {
        ...
        ch.open("get", `${path}/storeshow?click='ok'&store=${city}&changemonth=${current_month}`, true);
        ch.send(null);
        ch.onload = function () {
            if (ch.status == 200) {
