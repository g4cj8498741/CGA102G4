...
public class ReDay {
	...
	public void day(int start, int end, int sumMonth, int month, String startstore, String endstore) {
...
//		}
	...
	public void otherStoreCar(int start, int end, int endmonth, int month, String startstore, String endstore,String actual ,int rrEndTime) { ////////新增
		List<Integer> day = new ArrayList<>();
...
//				}
			...
			} else if (endmonth > month) { //訂單結束在下一個月份 應為外站 會跨月份
				int date = monthinDay(month);
				while (start <= date) { //當月份天數
					day.add(start);
					start++;
				}
				for (int i = 1; i <= end; i++) { //跨月 天數
					//nextday.add(i);
					day.add(i);
				}
